...
#ifndef OPTIMIST_NONLINEAR_SOLVER_HXX
#define OPTIMIST_NONLINEAR_SOLVER_HXX
  ...
  */
  ...
  public:
    ...
    */
    void tolerance(Real t_tolerance) {
      OPTIMIST_ASSERT(
        ...
        "Optimist::NonlinearSolver::tolerance(...): invalid input detected.");
    ...
    */
    void max_iterations(Integer t_max_iterations) {
      OPTIMIST_ASSERT(!std::isnan(t_max_iterations) && std::isfinite(t_max_iterations),
        "Optimist::NonlinearSolver::max_iterations(...): invalid input detected.");
    ...
    */
    ...
    {
      OPTIMIST_ASSERT(!std::isnan(t_max_fun_evaluations) && std::isfinite(t_max_fun_evaluations),
        "Optimist::NonlinearSolver::max_function_evaluations(...): invalid input detected.");
    ...
    */
    ...
    {
      OPTIMIST_ASSERT(!std::isnan(t_max_jac_evaluations) && std::isfinite(t_max_jac_evaluations),
        "Optimist::NonlinearSolver::max_jacobian_evaluations(...): invalid input detected.");
    ...
    */
    void max_relaxations(Integer t_max_relaxations)
    {
      OPTIMIST_ASSERT(!std::isnan(t_max_relaxations) && std::isfinite(t_max_relaxations),
        "Optimist::NonlinearSolver::max_relaxations(...): invalid input detected.");
    ...
    */
    void alpha(Real t_alpha)
    {
      OPTIMIST_ASSERT(!std::isnan(t_alpha) && std::isfinite(t_alpha) && Real(0.0) <= t_alpha && t_alpha <= Real(1.0),
        "Optimist::NonlinearSolver::alpha(...): invalid input detected.");
...
#endif // OPTIMIST_NONLINEAR_SOLVER_HXX
