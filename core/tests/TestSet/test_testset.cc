...
using ScalarTestTypes = testing::Types<Linear<float>,
                                       Linear<double>,
                                       Quadratic<float>,
                                       Quadratic<double>,
                                       Cos<float>,
...
TYPED_TEST(ScalarFunctions, Solve) {
  ...
  for (size_t i{0}; i < fun.guesses().size(); ++i) {
    ...
    // Check convergence
    if (sol.converged() &&
        fun.is_solution(x_out, std::cbrt(Function::EPSILON))) {
...
using VectorTestTypes =
    testing::Types<Linear1<Eigen::Vector<float, 1>>,
...
using CostTestTypes = testing::Types<
    ...
    // Brown<Eigen::Vector<float, Eigen::Dynamic>, Eigen::Vector<float,
    // Eigen::Dynamic>>, Brown<Eigen::SparseVector<float>,
    // Eigen::SparseVector<float>>, Brown<Eigen::Vector<double, 2>,
...
//     // Check convergence
//     if (sol.converged() && fun.is_solution(x_out,
//     std::cbrt(Function::EPSILON))) {
//       converged_at_least_once = true;
//     }
//   }
//   ASSERT_TRUE(converged_at_least_once);
