...
TYPED_TEST(Functions, Solve) {
  ...
  Function fun;
  Optimist::RootFinder::Broyden<Vector> sol;
  sol.task(fun.name());
  sol.verbose_mode(false);
  sol.tolerance(std::sqrt(Function::EPSILON));

  // Define methods to test
  using Method = typename Optimist::RootFinder::Broyden<Vector>::Method;
  auto methods = {Method::GOOD, Method::BAD, Method::COMBINED};
