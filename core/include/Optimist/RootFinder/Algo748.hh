...
namespace Optimist {
  namespace RootFinder {
     ...
     *
     * The algorithm 748 allows to find the roots of a scalar function
     * \f$f(x)\f$ in a given interval
     * \f$[a, b]\f$. The algorithm is based on the work of G. Alefeld, F. Potra,
     * and Y. Shi, *Algorithm 748: Enclosing Zeros of Continuous Functions*, ACM
     * Transactions on Mathematical Software, 21 (1995), pp.
     * 327-344, 10.1145/210089.210111.
    ...
    class Algo748 : public Bracketing<Scalar, Algo748<Scalar>> {
     ...
     private:
       ...
       * \return The boolean flag, true if all the values are different, false
       * otherwise.
       */
      ...
      /**
       * Given current enclosing interval \f$[a, b]\f$ and \f$a\f$ number
       * \f$c\f$ in
       * \f$(a, b)\f$:
       *   1. if \f$f(c) = 0\f$ then sets the output \f$a = c\f$.
       *   2. Otherwise determines the new enclosing interval \f$[a, b] = [a,
       * c]\f$ or \f$[a, b] = [c, b]\f$. also updates the termination criterion
       * corresponding to the new enclosing interval. Adjust \f$c\f$ if
       * \f$(b-a)\f$ is very small or if \f$c\f$ is very close to \f$a\f$ or
       * \f$b\f$.
...
#undef CMD
      ...
      /**
       * Perform the cubic inverse interpolation of \f$f(x)\f$ at \f$a\f$,
       * \f$b\f$,
       * \f$d\f$, and
      ...
      /**
       * Perform \f$n\f$ Newton steps to approximate the zero in \f$(a, b)\f$ of
       * the quadratic polynomial interpolating \f$f(x)\f$ at \f$a\f$, \f$b\f$,
       * and
       * \f$d\f$. Safeguard is used to avoid overflow.
     ...
     public:
       ...
       * equation
       * \f$f(x) = 0\f$ in the interval \f$[a,b]\f$. At the beginning of each
       * iteration, the current enclosing interval is recorded as \f$[a_0,
       * b_0]\f$. The first iteration is simply a secant step. Starting with the
       * second iteration, three steps are taken in each iteration.
       *   1. The first two steps are either quadratic interpolation or cubic
       * inverse interpolation.
       *   2. The third step is a double-size secant step.
       * If the diameter of the enclosing interval obtained after these three
       * steps is larger than
       * \f$\mu*(b_0-a_0)\f$, an additional bisection step will be used to
       * shrink the enclosing interval.
